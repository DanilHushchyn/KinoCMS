...
class Command(BaseCommand):
    ...
    def _create_sliders(cls):
        ...
        if not ETEndBBanner.objects.exists():
            ETEndBBanner.objects.create(
                color="#FFFFFF", use_img=False, image=cls._create_image("bottom_slider")
            )
    ...
    def _create_image(cls, seed_path: str) -> Image:
        ...
        """
        random_image = random.choice(os.listdir(os.path.join("seed", seed_path)))
    ...
    def create_hall_schema(cls) -> dict:
        random_schema = random.choice(os.listdir(os.path.join("seed", "hall_schemas")))
        ...
        if not NewsPromo.objects.exists():
            ...
            for i in range(1, 101):
                if i % 2 == 0:
                    ...
                    promo = False
                description_uk = cls._fake_uk.text(max_nb_chars=2500).capitalize()
                description_ru = cls._fake_ru.text(max_nb_chars=2500).capitalize()
    ...
    def _create_schedule(cls, start_date, end_date, movie, price, hall_id):
        ...
        seances = []
        for i in range(1, 31):
            date = cls._fake_uk.date_time_between(
    ...
    def _create_seances(cls, movie: Movie):
        if not movie.seance_set.exists():
            ...
            for hall_id in hall_ids:
                price = random.choice([150, 100, 250, 500, 280, 180, 600, 350, 420])
    ...
    def _create_participants(cls) -> None:
        ...
        if not MovieParticipantRole.objects.exists():
            MovieParticipantRole.objects.bulk_create(
                [
                    MovieParticipantRole(name_uk="актор", name_ru="актёр"),
                    MovieParticipantRole(name_uk="продюсер", name_ru="продюсер"),
                    MovieParticipantRole(name_uk="режисер", name_ru="режисёр"),
                    MovieParticipantRole(name_uk="сценарист", name_ru="сценарист"),
                    MovieParticipantRole(name_uk="композитор", name_ru="композитор"),
                ]
            )
        role_ids = list(MovieParticipantRole.objects.values_list("id", flat=True))
        ...
        person_ids = list(MovieParticipantPerson.objects.values_list("id", flat=True))
        if not MovieParticipant.objects.exists():
            participants = []
            for person_id in person_ids:
                participants.append(
                    MovieParticipant(
                        role_id=random.choice(role_ids), person_id=person_id
                    )
                )
    ...
    def _create_movies(cls):
        if not Movie.objects.exists():
            ...
            for i in range(1, 41):
                ...
                description_ru = cls._fake_ru.text(max_nb_chars=2500).capitalize()
                if i % 2 == 0:
                    released = cls._fake_uk.date_this_month(
            ...
            participant_ids = list(
                MovieParticipant.objects.values_list("id", flat=True)
            )
