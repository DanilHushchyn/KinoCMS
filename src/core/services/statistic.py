...
class StatisticService:
    ...
    def get_computed_nums(
        ...
        current_month_tickets = (Ticket.objects
                                 .filter(date_created__range=
                                         [start_current_month,
                                          end_current_month], )
                                 )
        current_month_income = (current_month_tickets
        .aggregate(total_income=Sum('seance__price'))
        ['total_income'])
        last_month_tickets = (Ticket.objects
                              .filter(date_created__range=
                                      [start_last_month,
                                       end_last_month], )
                              )
        last_month_income = (last_month_tickets
        .aggregate(total_income=Sum('seance__price'))
        ['total_income'])
        income_progress = self.progress_calc(current_month_income,
                                             last_month_income)
        income_progress = 0 if income_progress is None \
            else round(income_progress, 2)
        result = {
    ...
    def get_most_popular_movies(
        ...
        today = pendulum.now(tz='Europe/Kiev')
        start_current_month = today.start_of('month')
        tickets = (Ticket.objects.prefetch_related('seance__movie')
    ...
    def get_most_income_movies(
        ...
        values = []
        for label in labels:
            value = (tickets.filter(seance__movie__name_uk=label)
                     .aggregate(total_income=Sum('seance__price')))
    ...
    def get_most_popular_techs(
        ...
        values = []
        for label in labels:
            value = (tickets.filter(seance__hall__tech__name=label)
                     .count())
            values.append(value)
        total = sum(values)
        values_percents = []
