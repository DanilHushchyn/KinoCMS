...
class NewsPromoInSchema(ninja_schema.ModelSchema):
    ...
    def clean_tags(cls, tag_ids: list[int]) -> list[int]:
        ...
        tags_db = list(Tag.objects.filter(id__in=tag_ids).values_list("id", flat=True))
        if len(tag_ids) != len(tags_db):
            diff = list(set(tag_ids) ^ set(tags_db))
            msg = _(
                "У заданому переліку тегів є " "ids {diff} які не присутні у базі"
            ).format(diff=diff)
            raise NotFoundExceptionError(message=msg, cls_model=Tag, field="tags")
        return tag_ids

    class Config:
        model = NewsPromo
        exclude = ["id", "name", "description", "slug", "date_created"]
        optional = [
            "gallery",
        ]
...
class NewsPromoUpdateSchema(ninja_schema.ModelSchema):
    ...
    class Config:
        model = NewsPromo
        exclude = ["id", "name", "description", "promo", "slug", "date_created"]
        optional = "__all__"
