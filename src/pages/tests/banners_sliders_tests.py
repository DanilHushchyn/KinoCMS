...
from src.pages.models import TopSliderItem, BottomSliderItem
...
class TestBannersSlidersController:
    ...
    def test_get_top_slider(self):
        response = self.client.get(f"/top/",
                                   headers=self.headers)
        assert response.status_code == 200

    def test_get_bottom_slider(self):
        response = self.client.get(f"/bottom/",
                                   headers=self.headers)
        assert response.status_code == 200

    def test_get_etend_banner(self):
        response = self.client.get(f"/etend-banner/",
                                   headers=self.headers)
        assert response.status_code == 200
    ...
    def test_create_top_slider_item(self):
        ...
        # test create
        response = self.client.patch(f"/top/",
                                     json={
                                         "items": [
                                             {
                                                 "image": {
    ...
    def test_create_bottom_slider_item(self):
        ...
        # test create
        response = self.client.patch(f"/bottom/",
                                     json={
                                         "items": [
                                             {
                                                 "image": {
    ...
    def test_delete_bottom_slider_item(self):
        item_ids = list(BottomSliderItem.objects
    ...
    def test_update_etend_banner(self, payload, expected_status):
        response = self.client.patch(f"/etend-banner/",
                                     json=payload,
                                     headers=self.headers)
