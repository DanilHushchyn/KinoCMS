"""Module for testing news and promos"""
...
class TestNewsPromoController:
    ...
    def test_get_news_promo(self, np_slug, expected_status):
        response = self.client.get(f"/{np_slug}/", headers=self.headers)
    ...
    def test_get_all_news_promo_cards(self):
        response = self.client.get("/all-cards/?promo=true", headers=self.headers)
        assert response.status_code == 200
        response = self.client.get("/all-cards/?promo=false", headers=self.headers)
        assert response.status_code == 200

    def test_get_all_news_promo_tags(self):
        response = self.client.get("/all-tags/", headers=self.headers)
        assert response.status_code == 200

    @pytest.mark.parametrize(
        "np_slug, payload, expected_status",
        [
            (
                "novina-05",
                {
    ...
    def test_update_news_promo(self, np_slug, payload, expected_status):
        response = self.client.patch(f"/{np_slug}/", json=payload, headers=self.headers)
        assert response.status_code == expected_status
    ...
    def test_delete_news_promo(self, np_slug, expected_status):
        response = self.client.delete(f"/{np_slug}/", headers=self.headers)
        assert response.status_code == expected_status
