...
class TestMailingController:
    ...
    def test_create_delete_template(self, expected_status):
        random_template = random.choice(os.listdir(os.path.join("seed", "template")))
        ...
        with open(file_path, "rb") as f:
            ...
            files = {
                "file": UploadedFile(file_content, content_type="text/html", size=size)
            }
    ...
    def test_get_templates(self, expected_status):
        response = self.client.get("/templates/", headers=self.headers)
        assert response.status_code == expected_status
    ...
    def test_mailing(
        ...
        template = MailTemplate.objects.create(
            name="mailing-template", file=File(file, "/media/MailTemplate/" + file.name)
        )
        mailing_response = self.client.post(
            "/start/",
            json={"temp_id": template.id, "user_ids": [1, 2, 3]},
            headers=self.headers,
        )
