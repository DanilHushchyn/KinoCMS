"""Module for testing essence User"""
...
class TestUsersAdminController:
    """Controller class for testing essence User"""
    ...
    def test_delete_user_by_id(self, user_id, expected_status) -> None:
        """Test delete user by id
        :param user_id: user id
        :param expected_status: expected_status
        :return: None
        """
        response = self.client.delete(f"/detail/{user_id}/", headers=self.headers)
    ...
    def test_get_user_by_id(self, payload, expected_status):
        ...
        response = self.client.get(
            f"/detail/{payload['user_id']}/", headers=self.headers
        )
        assert response.status_code == expected_status
        if response.status_code == 200:
            response_encoded = json.dumps(response.json(), ensure_ascii=False)
    ...
    @pytest.mark.parametrize(
        "user_id,payload,expected_status",
        [
            (
                1,
                {
    ...
    def test_update_user_by_id(self, user_id, payload, expected_status):
        ...
        response = self.client.patch(
            f"/detail/{user_id}/", json=payload, headers=self.headers
        )
        assert response.status_code == expected_status
    ...
    def test_datable(self, queries, expected_status):
        """Test get datable
        :param queries: kit of query parameters for filtering
        :param expected_status: expected_status
        :return: None
        """
        response = self.client.get(f"/datable/{queries}", headers=self.headers)
        assert response.status_code == expected_status
