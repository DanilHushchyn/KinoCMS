...
class TestHallAdminController:
    ...
    @pytest.mark.parametrize("hall_id,expected_status",
                             [
                                 (
    ...
    def test_get_hall(self, hall_id, expected_status):
        response = self.client.get(f"/{hall_id}/",
    ...
    def test_create_hall(self, payload, expected_status):
        response = self.client.post(f"/?cnm_slug=knoteatr-01", json=payload,
                                    headers=self.headers)
        assert response.status_code == expected_status

    @pytest.mark.parametrize("hall_id, payload, expected_status",
                             [
                                 (
                                         1,
                                         {
    ...
    def test_update_hall(self, hall_id, payload, expected_status):
        response = self.client.patch(f"/{hall_id}/", json=payload,
                                     headers=self.headers)
        assert response.status_code == expected_status
    ...
    def test_get_all_hall_cards(self, cnm_slug, expected_status):
        response = self.client.get(f"/all-cards/?cnm_slug={cnm_slug}",
                                   headers=self.headers)
        assert response.status_code == expected_status
    ...
    def test_delete_hall(self, hall_id, expected_status):
        response = self.client.delete(f"/{hall_id}/",
                                      headers=self.headers)
        assert response.status_code == expected_status
